...
class SBox:
    ...
    def __call__(self, binary: str) -> str:
        ...
        # Look up value in S-box table
        if (a, b) in self.table:
            return int_to_bin(self.table[(a, b)], block_size=self.block_size)  # Return 4-bit result
        else:
            return binary  # Fallback 

    @staticmethod
    def des_single_round_substitutions():
        # Return list of 8 S-boxes (one for each block in DES)
        return [SBox.for_des_substitution(block) for block in range(1, 9)]

    @staticmethod
    def identity():
        # A dummy SBox that returns '0000' for any input
        return SBox(func=lambda binary: ('0', '0'), table={})

    @staticmethod
    def for_des_substitution(block):
        # Return corresponding DES S-box based on block number
    ...
    def des_confusion(binary: str) -> tuple:
        # Extract row and column from a 6-bit input
        return binary[0] + binary[5], binary[1: 5]

    @staticmethod
    def from_list(sequence: list):
        # Convert a 4x16 S-box list into a dictionary mapping (row, col) → value
        mapping = {}
